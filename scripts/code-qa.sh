...
CLAUDE.md COMPLIANCE
  ...
  - No inline styles.
  - components/ui/ files: existing ones not hand-edited. **New shadcn primitives are allowed** under the CLAUDE.md §15 / file-ownership-rules carve-out when (a) the PR description lists each primitive added and the criterion it serves, and (b) the file was generated by \`npx shadcn add <name>\` (not hand-written).
  - **Recognition note for current shadcn output:** the current shadcn CLI (since late 2024) emits imports from the **unified \`radix-ui\` package** (e.g. \`import * as DialogPrimitive from "radix-ui"\`), NOT the old per-component \`@radix-ui/react-<name>\` pattern. The \`radix-ui\` import is the **canonical shadcn-CLI output**, not evidence of a hand-written file. Do not flag it as a hand-write violation.
...
5. **For tickets with PROMPT_FILE_TOUCHED == true:** include a section titled "lib/prompt.ts — entire file before/after" that pastes the **complete contents** of PROMPT_BEFORE and PROMPT_AFTER as provided to you above. Two fenced code blocks labelled BEFORE and AFTER. **Paste the WHOLE FILE — every line, every export, every helper function, including imports.** Do NOT scope to a single named export. Quoting only `SYSTEM_PROMPT` and skipping `buildUserPrompt` (or vice versa) is wrong: prompt logic can live in either, and a narrow artifact masks the change. (Rule earned the hard way during INS-15, where a narrow artifact nearly triggered a wrong revert because the reframe lived in `buildUserPrompt`, not `SYSTEM_PROMPT`.) The human reads the English text to confirm intended prose changes are real and unintended prose changes are absent.
